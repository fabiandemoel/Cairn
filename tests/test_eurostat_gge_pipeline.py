"""Tests for the Eurostat env_air_gge ingestion pipeline.
...
from ingestion import eurostat_gge_pipeline as ep
from ingestion.manifest import Manifest, Snapshot, compute_sha256, save_manifest

# Minimal SDMX-CSV matching the Eurostat env_air_gge format.
# Three countries, two CRF sectors, three years.
FIXTURE_CSV = (
    "DATAFLOW,LAST UPDATE,freq,unit,airpol,src_crf,geo,TIME_PERIOD,OBS_VALUE,OBS_FLAG,CONF_STATUS\n"
    "ESTAT:ENV_AIR_GGE(1.0),02/06/26 23:00:00,A,MIO_T,GHG,TOTXMEMO,NL,2022,167.80468,,\n"
    "ESTAT:ENV_AIR_GGE(1.0),02/06/26 23:00:00,A,MIO_T,GHG,TOTXMEMO,NL,2023,158.3252,,\n"
    "ESTAT:ENV_AIR_GGE(1.0),02/06/26 23:00:00,A,MIO_T,GHG,TOTXMEMO,NL,2024,154.21476,,\n"
    "ESTAT:ENV_AIR_GGE(1.0),02/06/26 23:00:00,A,MIO_T,GHG,CRF1,NL,2022,94.85654,,\n"
    "ESTAT:ENV_AIR_GGE(1.0),02/06/26 23:00:00,A,MIO_T,GHG,TOTXMEMO,DE,2022,746.98888,,\n"
    "ESTAT:ENV_AIR_GGE(1.0),02/06/26 23:00:00,A,MIO_T,GHG,TOTXMEMO,FR,2024,383.47025,,\n"
)
...
def test_parse_release_dd_mm_yyyy() -> None:
    assert ep._parse_release("02.06.2026") == "2026-06-02"


def test_parse_release_iso_date() -> None:
    assert ep._parse_release("2026-06-02") == "2026-06-02"


def test_parse_release_iso_date_with_time() -> None:
    assert ep._parse_release("2026-06-02T23:00:00+0200") == "2026-06-02"


def test_parse_release_iso_month() -> None:
    assert ep._parse_release("2026-06") == "2026-06"


def test_parse_release_rejects_unknown() -> None:
    with pytest.raises(ValueError, match="Cannot parse"):
        ep._parse_release("Jun 2026")
...
def test_extract_update_date_finds_update_data_annotation() -> None:
    payload = {
        "extension": {
            "annotation": [
                {"type": "CREATED", "date": "2015-12-07T08:50:25+0100"},
                {"type": "UPDATE_DATA", "date": "2026-06-02T23:00:00+0200"},
                {"type": "UPDATE_STRUCTURE", "date": "2026-06-02T23:00:00+0200"},
            ]
        }
    }
    assert ep._extract_update_date(payload) == "2026-06-02T23:00:00+0200"


def test_extract_update_date_missing_annotation_raises() -> None:
    payload = {"extension": {"annotation": [{"type": "CREATED", "date": "2015-12-07"}]}}
...
def test_periods_covered(tmp_path: Path) -> None:
    ...
    csv_path.write_text(FIXTURE_CSV, encoding="utf-8")
    dest = ep._export_parquet(csv_path, tmp_path / "release")
    assert ep._periods_covered(dest) == ["2022", "2024"]
