...
The README's Eurostat section (`env_ac_aibrid_r2`, README.md "EU sector data") and
`benchmark_country_sector_emissions`'s own model doc already cite Eurostat's *bridge
between air emissions accounts and national inventories* as "the documented
explanation" for why AEA's residence-principle totals legitimately differ from CBS
85669NED's and the EU ETS's territorial-principle totals — then add, verbatim: "it is
not ingested." This candidate closes that self-identified gap by surfacing Eurostat's
own official bridge items instead of Cairn approximating a residence-vs-territorial
reconciliation itself (which would be recompute-adjacent, exactly what `mart_namea_bridge`
had to be built carefully to avoid).
- *Layers:*
  ...
  - mart+site (fused) — `mart_eurostat_aibrid_bridge`: grain country × bridge-item × year,
    ...
    "corrected" total — that is precisely the recomputation this candidate exists to avoid).
    Site query + a new section on `/sectors-eu` and/or `/countries-ghg` quantifying, with real
    numbers, the residence-vs-territorial gap those pages currently narrate only qualitatively.
- *Watch:* this is Eurostat's own official reconciliation, not one Cairn derives — never
  compute a "corrected" territorial total from the bridge items, and never join it into
  `benchmark_country_sector_emissions` or `mart_gge_national_totals` as an adjustment; it
  stands alone as the documented residual. Once ingested, update the README's
...
own model doc already names the intent: the reviewed `lei_mapping_euets` seed (GLEIF/LEI
mapping, shipped PR #66) joins "letting the benchmark roll up to company level" — a rollup
that was never actually built. Several NL ETS operators run more than one installation
under one legal entity; grouping verified emissions by the *reviewed* GLEIF identifier —
never euets.info's own unreviewed free-text `parent_company` field, which the mart doc
explicitly flags as "not normalised or deduplicated" — is a genuinely new benchmark cut
(corporate footprint vs. installation/sector footprint) at zero new-ingestion cost.
- *Layers:*
  - mart+site (fused) — `mart_corporate_group_emissions`: grain `lei` × year, summing
    `verified_emissions_t_co2eq`, `allocated_total_t_co2eq`, and
    `surrendered_allowances_t_co2eq` across every installation sharing an LEI, plus the
    installation count and `gleif_legal_name` per group. Installations with no LEI match
    are excluded (never grouped into a fake "unmatched" bucket) — the excluded share is
    already visible via `mart_field_completeness`'s `lei` column, so this mart doesn't
    need to restate it. Site query + a new section on `/installations` (a group rollup
    table alongside the existing per-installation ranking).
- *Watch:* group strictly on `lei`, never on `parent_company` — grouping on free text would
  silently fragment or merge groups on spelling variance. This sums already-verified figures
  along an existing, reviewed dimension; it is not a new estimate. `lei_mapping_euets`
  currently covers only a small, hand-reviewed slice of installations (15 rows as of
  2026-07-30) — the rollup will legitimately cover few companies at first and grow only as
  the seed does; don't backfill or infer missing matches to make the rollup look fuller.
...
manual euets refresh) the 2026-07-15 external review identified — the other three
(homepage IA, the NACE-coverage disclosure, and glossary cross-links) shipped in
PR #140 (2026-07-15) and have moved to _Considered and rejected_. Unlike the
expansion candidates above, these add **no** new benchmark axis, so they sit outside
the value ordering the expansion candidates follow and are kept in the review's
...
- **Eurostat GGE CRF-sector cross-country benchmark — IPCC sectoral cross-check.**
  ...
  `sources/eurostat_gge/manifest.yml`; the CRF top-level sector breakdown
