...
README's References section already cites Eurostat dataset `env_ac_aibrid_r2` —
*Bridge between air emissions accounts and national inventories* — as "the
documented explanation" for why the AEA's residence-principle totals
(`benchmark_country_sector_emissions`) legitimately diverge from CBS 85669NED's
and the EU ETS's territorial-principle totals, then states plainly: "it is not
ingested." This candidate closes that self-identified gap. Rather than Cairn
deriving its own residence-vs-territorial reconciliation (recompute-adjacent —
exactly the trap `mart_namea_bridge` had to be built carefully to avoid), it
surfaces Eurostat's own official bridge items, for every EU member state, at
zero methodology risk.
- *Layers:*
  - ingestion — a new Eurostat SDMX source, mirroring
    `ingestion/eurostat_aea_pipeline.py`'s metadata-driven freshness check (same
    API family, same `METADATA_URL` pattern) against `env_ac_aibrid_r2`. Confirm
    the exact bridge-item codes (e.g. resident-producers'-emissions-abroad vs.
    non-residents'-emissions-in-territory, by transport mode) against the real
    downloaded dataset before coding — do not assume the item taxonomy from this
    description alone.
  - staging — `stg_eurostat_aibrid__bridge_items`, a 1:1 typed view, mirroring
    `stg_eurostat__aea`'s pattern.
  - mart+site (fused) — `mart_eurostat_aibrid_bridge`: grain country × bridge
    item × year, GHG only, each bridge item kept as its own read/relabel row (no
    netting into a single "corrected" total — that is precisely the
    recomputation this candidate exists to avoid). Site query + a new section on
    `/sectors-eu` and/or `/countries-ghg` quantifying, with real numbers, the
    residence-vs-territorial gap those pages currently narrate only
    qualitatively.
- *Watch:* this is Eurostat's own official reconciliation, not one Cairn
  derives — never compute a "corrected" territorial total from the bridge
  items, and never join it into `benchmark_country_sector_emissions` or
  `mart_gge_national_totals` as an adjustment; it stands alone as the
  documented residual. Once ingested, update the README's `env_ac_aibrid_r2`
  citation (currently "it is not ingested") in the same PR.
...
Every published benchmark is installation-level, but `benchmark_installation_emissions`'s
own model doc already names the intent of the GLEIF join: "letting the
benchmark roll up to company level" — a rollup that was never actually built.
Several NL ETS operators run more than one installation under one legal
entity; grouping verified emissions by the *reviewed* GLEIF identifier — never
euets.info's own unreviewed free-text `parent_company` field, which the mart
doc explicitly flags as "not normalised or deduplicated across installations"
— is a genuinely new benchmark cut (corporate footprint vs. installation/sector
footprint) at zero new-ingestion cost.
- *Layers:*
  - mart+site (fused) — `mart_corporate_group_emissions`: grain `lei` × year,
    summing `verified_emissions_t_co2eq`, `allocated_total`, and `surrendered`
    across every installation sharing an LEI, plus the installation count and
    `gleif_legal_name` per group. Installations with no LEI match are excluded
    (never grouped into a fake "unmatched" bucket) — the excluded share is
    already visible via `mart_field_completeness`'s `lei` column, so this mart
    doesn't need to restate it. Site query + a new section on `/installations`
    (a group rollup table alongside the existing per-installation ranking).
- *Watch:* group strictly on `lei`, never on `parent_company` — grouping on
  free text would silently fragment or merge groups on spelling variance. This
  sums already-verified figures along an existing, reviewed dimension; it is
  not a new estimate. `lei_mapping_euets` currently carries only 15 reviewed
  rows with 10 distinct non-null LEIs (as of 2026-08-06), and none of those 10
  yet repeats across installations in the seed — so the rollup will show only
  single-installation "groups" at first and gain real multi-installation
  groups only as the seed grows. Don't backfill or infer missing matches to
  make the rollup look fuller; that's the seed's job, reviewed one PR at a
  time.
...
identified. The other three — the homepage information architecture, the
NACE-coverage disclosure, and the glossary cross-links — shipped in PR #140
(2026-07-15) and have moved to _Considered and rejected_. Unlike the expansion
candidates above, these add **no** new benchmark axis, so they sit outside the
value ordering the expansion candidates follow and are kept in the review's
original P1→P2 order. Dispatch reality: the no-LLM dispatcher opens at most one
issue per run for the *top* candidate's next not-yet-built layer, and only a
**new-file** sentinel triggers it — so entry 5 (new methodology source query) and
entry 7 (new og:image asset) are genuinely dispatchable, while the manual euets
refresh (entry 6) carries an existing-file sentinel the dispatcher treats as
already-present and skips. Work that one (or 5/7 sooner than the queue reaches
them) by raising the GitHub issue by hand and labelling it `approved` — the
supported manual implement path.
...
- **Eurostat GGE CRF-sector cross-country benchmark — IPCC sectoral cross-check.**
  ...
  `sources/eurostat_gge/manifest.yml`; the top-level CRF sector breakdown
  (Energy, Industrial processes and product use, Agriculture, Waste, LULUCF) is
  surfaced in a new section on the country GHG page (`countries-ghg.md`).
