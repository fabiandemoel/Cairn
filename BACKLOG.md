...
### 2. Emissieregistratie ↔ CBS reconciliation — surface the already-computed cross-check
<!-- dispatch
layers:
  site: site/sources/cairn/emissieregistratie_reconciliation.sql
-->
**Value: M · Effort: L · Spine-fit: H**

`mart_emissieregistratie_cbs_reconciliation` (shipped 2026-07-08) cross-checks
RIVM's UNFCCC CRF national total against CBS 85669NED's national total,
guarded by `assert_emissieregistratie_nl_total_within_cbs` — but unlike the
coverage, freshness, and field-completeness marts (all surfaced on
`/data-quality`), it has **no site query at all**. The read-only site's whole
point is to expose what the warehouse already computes; a fully-tested
cross-check that's invisible on the read-only layer is a presentation gap,
not a data gap.
- *Layers:*
  - site — a new `site/sources/cairn/emissieregistratie_reconciliation.sql`
    query over `mart_emissieregistratie_cbs_reconciliation` (year, both
    totals, `relative_deviation`), and a new section on `/data-quality`
    alongside the existing coverage/freshness/completeness sections,
    presenting it the same way — a plain table or small chart, with the
    mart's own documented caveat (CBS revises more often than the annual
    UNFCCC cycle, so a multi-percent gap is expected, not a fault) carried
    into the page copy. Guard: `evidence-build` green.
- *Watch:* site-only — no mart or manifest changes, no new tolerance logic
  (the 10% test already exists). The mart's own known-limitations note that
  the CI fixture is a minimal excerpt (so the reconciliation test vacuously
  passes in CI until a fuller snapshot is ingested) is a pre-existing,
  accepted quirk — don't try to "fix" it here.

### 3. Eurostat GGE gas composition — cross-country GHG-by-gas breakdown
<!-- dispatch
layers:
  mart+site: transform/models/marts/mart_gge_gas_composition.sql; site/sources/cairn/gge_gas_composition.sql
-->
**Value: M · Effort: L · Spine-fit: H**

`stg_eurostat__gge` already stages every `airpol` (air pollutant) code the
pinned env_air_gge snapshot carries, not just the `GHG` aggregate
`mart_gge_national_totals` reads. Eurostat's own AIRPOL codelist confirms the
dataset natively splits the aggregate into CO2-equivalent gas codes (`CO2`,
`CH4_CO2E`, `N2O_CO2E`, `HFC_CO2E`, `PFC_CO2E`, `SF6_CO2E`, `NF3_CO2E`)
alongside `GHG` itself. Surfacing that split is a genuinely new axis — a
gas-composition benchmark at EU cross-country scale, independent of Cairn's
two existing gas-composition marts (`mart_sector_gas_composition`,
`mart_namea_gas_composition`), both NL-only and both ultimately CBS-sourced.
Zero new-ingestion cost — the source is already pinned.
- *Layers:*
  - mart+site (fused) — a new mart reading `stg_eurostat__gge` filtered to
    `src_crf = 'TOTXMEMO'`, `unit = 'MIO_T'`, `airpol` restricted to the
    CO2-equivalent gas codes (confirm the exact code list against the pinned
    parquet before hardcoding a `where airpol in (...)` — Eurostat's codelist
    documents them summing to `GHG`, but verify against the real snapshot's
    populated codes, not every code Eurostat defines), grain country × gas ×
    year, NULL `obs_value` cells excluded (never zero-filled). Site query + a
    new section on the country GHG page (`countries-ghg.md`), alongside the
    existing national totals and the CRF-sector section.
- *Watch:* the `airpol` gas codes are Eurostat's own CO2-equivalent
  aggregates (e.g. `CH4_CO2E` already folds CH4 into CO2-eq) — read/relabel
  only, never re-derive or apply a GWP conversion. Never cross-walk this to
  CBS's or NAMEA's gas breakdown as if they were the same measurement;
  present it as an independent Eurostat-sourced view, the same caveat already
  applied to `mart_gge_sector_totals` vs NACE.
...
### 5. EEA EU ETS main-activity cross-country benchmark
<!-- dispatch
layers:
  mart+site: transform/models/marts/mart_eea_activity_totals.sql; site/sources/cairn/eea_activity_totals.sql
-->
**Value: M · Effort: M · Spine-fit: H**

`ingestion/eea_ets_pipeline.py`'s own module docstring describes the pinned EEA
Union Registry bulk as "emissions, allowances and surrendered units by
**country x main activity x year**" — every EU/EEA country, not just NL. Cairn
today reads this source for exactly one purpose: the NL stationary aggregate
(`main_activity_code = '20-99'`) that `assert_euets_coverage_within_eea`
cross-checks against. The same pinned file's cross-country and per-activity
dimensions are otherwise unused. Surfacing them is a new axis: verified EU ETS
emissions by activity type, across every EU/EEA country, from the EU's own
official registry aggregate — a third classification system alongside NACE
(`benchmark_country_sector_emissions`) and IPCC/CRF (`mart_gge_sector_totals`),
and the only one native to the ETS itself. Zero new-ingestion cost.
- *Layers:*
  - mart+site (fused) — a new mart reading `stg_eea__ets` filtered to
    `citl_information = '2. Verified emissions'` (the same category the
    coverage test uses). First confirm, against the real pinned parquet,
    whether `main_activity_code` carries individual per-activity codes beyond
    the `20-99` aggregate, and what the `size` dimension partitions (it is not
    yet used anywhere in the transform layer) — do not assume either from this
    description. If finer activity codes exist, grain country × activity ×
    year; if only the `20-99` aggregate is populated, grain country × year
    (still new: a cross-country verified-emissions time series has never been
    surfaced beyond the NL cross-check). The EEA zip also carries a
    "Translation of activity codes" workbook that Cairn's pipeline deliberately
    does not ingest (raw faithfulness keeps only the `ETS_Database` sheet) — if
    activity codes need labels, do not add that workbook as a second ingestion
    source; either present the raw codes or check whether they overlap with
    euets.info's already-pinned `dim_activity_type` labels (used in
    `stg_euets__installations`) before deciding. NULL values excluded, never
    zero-filled. Site query + a new section on the country GHG page
    (`countries-ghg.md`) or the EU sector page (`sectors-eu.md`) — pick
    whichever reads more naturally once the actual code granularity is known.
- *Watch:* this is the EEA **aggregate**, not installation-level data (CLAUDE.md's
  standing gotcha) — never join it row-for-row to `benchmark_installation_emissions`
  or treat it as a second installation pin. Main-activity codes are an EU ETS
  Annex I classification, not NACE and not CRF — never cross-walk to either.
  Read/relabel only.
...
Three entries remain below from an external review of the live site + repo (down
from six — the three P1 items covering homepage IA, NACE-coverage disclosure, and
glossary links shipped together in PR #140 and have been retired to "Considered and
rejected"). Unlike the expansion candidates above, these are correctness /
disclosure / presentation fixes (plus one manual euets refresh), one PR each — they
add **no** new benchmark axis, so they sit outside the value ordering the expansion
candidates follow and are kept in the review's P1→P2 order. Dispatch reality: the
`---` rule directly above this heading marks the end of the section
`scripts/dispatch.py`'s `parse_backlog` actually scans (it stops at the first bare
`---` line or the next `## ` heading after `## Live candidates`) — deliberately,
because this section's own un-numbered `### Site-review correctness & polish`
subheading would otherwise be picked up by the parser as a headless candidate with
no dispatch block and fail the guard test. Entries 6-8 below still carry
`<!-- dispatch -->` blocks for documentation and in case this section is ever
promoted above the rule, but as long as it stays here **none** of them are ever
read by `parse_backlog` or auto-dispatched by `cairn-dispatch.yml`, regardless of
their sentinel paths — work every one of them by raising the GitHub issue by hand
and labelling it `approved`, the supported manual implement path.

### 6. Methodology Sources table documents only 3 of 8 pinned sources (P1)
<!-- dispatch
layers:
...
### 7. euets pin is release 2024-10 (latest year 2023); 2024 verified emissions are long published (P2, data-refresh route)
<!-- dispatch
layers:
...
### 8. Site chrome: Evidence default `twitter:site @evidence_dev`, no og:image, chart palette off-brand (P2)
<!-- dispatch
layers:
...
- **Eurostat GGE CRF-sector cross-country benchmark — IPCC sectoral cross-check.**
  Shipped: merged in PR #135 (2026-07-15, issue #134). `mart_gge_sector_totals`
  and its `gge_sector_totals.sql` source query are live, surfacing GHG by
  top-level CRF sector and country on the country GHG page (`countries-ghg.md`).
- **Homepage IA — link every published page, fix misleading BigValue deltas.**
  Shipped: merged in PR #140 (2026-07-15). `index.md` now presents the NL
  benchmark spine first and an "EU context & cross-checks" tier for
  `/sectors-eu`, `/countries-ghg`, `/namea-bridge`, `/transport`; `/architecture`
  is linked from "Why it is auditable"; the misleading `▲ 2024` comparison slot
  and the "software vendors" audience line are gone.
- **NACE-coverage disclosure on `/installations`.** Shipped: merged in PR #140
  (2026-07-15). The source paragraph now discloses that installations without a
  NACE section in the pinned euets.info snapshot are excluded from the
  benchmark, linking `/data-quality`; the homepage's installation count caption
  reads "(stationary, NACE-mapped)" to signal it is a subset.
- **Glossary links from the five data pages.** Shipped: merged in PR #140
  (2026-07-15). `/sectors`, `/installations`, `/transport`, `/sectors-eu`, and
  `/countries-ghg` now link the first use of each glossary-defined term to
